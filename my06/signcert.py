...
def get_subject_cn(csr_der):
    ...
    requestInfo = decoder.decode(csr_der)[0][0]
    subject_name = requestInfo[1]
    for value in subject_name:
        if str(value[0][0]) == "2.5.4.3":
...
def issue_certificate(private_key_file, issuer, subject, pubkey):
    ...
    signed = sign(tbsCertificate,private_key_file)
...
print "[+] Issuing certificate for \"%s\"" % (subject_CN)
...
subject = asn1_sequence(
                asn1_set(
                    asn1_sequence(
                        ...
                        asn1_printablestring(subject_CN)
                    )
                )
            )
